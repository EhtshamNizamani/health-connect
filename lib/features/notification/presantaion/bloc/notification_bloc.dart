...
class NotificationBloc extends Bloc<NotificationEvent, NotificationState> {
  ...
  NotificationBloc(
    ...
    on<MarkNotificationsAsRead>(_onMarkNotificationsAsRead);
        on<UnreadCountUpdated>(_onUnreadCountUpdated); // <-- Naya handler register karein
  ...
  void _onStartListening(
    ...
    print("[NotificationBloc] 🚀 StartListeningToNotifications event received. Setting up listeners.");
    ...
    print("[NotificationBloc] 🔍 At the moment of listening, AuthBloc state is: ${initialState.runtimeType}");
    ...
    _authSubscription = _authBloc.stream.distinct((prev, next) {
      // Only fire if the user ID changes (from null to something, or vice versa)
      return prev.user?.id == next.user?.id;
    }).listen((authState) {
      print("[NotificationBloc] 📡 Auth state changed via stream. Firing event for user: ${authState.user?.id}");
  ...
  void _onAuthUserChanged(
    ...
    if (user != null) {
      print("[NotificationBloc] 👤 User is present (ID: ${user.id}). Subscribing to notifications.");
      
      _countSubscription = _getUnreadCountUseCase(user.id, user.role).listen((result) {
        result.fold(
          (failure) => print("[NotificationBloc] ❌ Error getting count: ${failure.message}"),
          (count) {
            print("[NotificationBloc] 🔢 New notification count: $count");
      ...
      print("[NotificationBloc] 🚪 User is null (logged out). Resetting state.");
  ...
  void _onUnreadCountUpdated(
    ...
    print("[NotificationBloc] 🔢 Handling new count: ${event.newCount}");
  ...
  Future<void> _onMarkNotificationsAsRead(
    ...
    print("[NotificationBloc] ✍️ Marking notifications as read for: ${user.id}");
