...
class ManageAvailabilityScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => sl<ManageAvailabilityBloc>()..add(LoadInitialSchedule()),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return DayAvailabilityCard(
                      ...
                      onRemoveTimeSlot: (slot) {
                        bloc.add(TimeSlotRemoved(day: day, slotToRemove: slot));
                      },
                    );
                  },
                ),
  ...
  Future<void> _showAddTimeSlotDialog(BuildContext context, String day, ManageAvailabilityBloc bloc) async {
    ...
    final String start = "${startTime.hour.toString().padLeft(2, '0')}:${startTime.minute.toString().padLeft(2, '0')}";
    final String end = "${endTime.hour.toString().padLeft(2, '0')}:${endTime.minute.toString().padLeft(2, '0')}";

    // Add the event to the BLoC
    bloc.add(TimeSlotAdded(day: day, newSlot: TimeSlot(startTime: start, endTime: end)));
  }
}
